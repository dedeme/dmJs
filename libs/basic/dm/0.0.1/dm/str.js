...
(() => {
  ...
  //# str - str
  str.ltrim = s => s.replace(/^\s+/, "");

  /// Remove trailing blanks
  //# str - str
  str.rtrim = s => s.replace(/\s+$/, "");

  /// Indicates if text[0] is a space. if text length is 0, returns false.
  //# str - bool
  str.isSpace = text => text.length === 0 ? false : text.charAt(0) <= ' ';
  ...
  //# str - str - num
  str.compare = (s1, s2) => s1 < s2 ? -1 : s1 > s2 ? 1 : 0;

  /// Returns s1 < s2 ? -1 : s1 > s2 ? 1 : 0; in locale
  //# str - str - num
  str.localeCompare = (s1, s2) => s1.localeCompare(s2);
  ...
  str.repeat = (s, times) => {
    let bf = "";
    for (let n = 0; n < times; ++n) bf += s;
    return bf;
  };

  /// Replaces olds by news in s
  //# str - str - str - str
  str.replace = (s, olds, news) => It.join(It.from(s.split(olds)), news);
