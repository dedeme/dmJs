...
goog.provide("Model");
goog.provide("SudokuDef");
goog.provide("SudokuData");
goog.provide("Data");
goog.provide("WorkerRequest");
goog.provide("WorkerResponse");
...
Data = class {
  ...
  static restore (serial) {
    return new Data (
      It.from(serial[0]).map(s => s === null ? s : SudokuDef.restore(s)).to(),
      It.from(serial[1]).map(s => SudokuData.restore(s)).to(),
      serial[2],
      serial[3],
      serial[4]
    );
