...
import Tracker from "./dmjs/Tracker.js";
...
export default class View {
  ...
  static imgMenu (img, tooltip, isGrey) {
    isGrey = isGrey || false;
    const r = isGrey
      ? View.tracker().grey(img).setStyle("vertical-align", "bottom")
    ...
    menu.removeAll().add(
      $("table").att("align", "center").add($("tr")
        ...
        .add($("td").klass("menu").style("vertical-align:middle")
          ...
          .add(View.imgMenu(String(Model.mdata().level()), ""))
  ...
  static mkEndMenu () {
    const tm = Model.convertScs(Model.last().time());
    menu.removeAll().add(
      $("table").att("align", "center").add($("tr")
        .add($("td").klass("menu")
  ...
  static mkNumberKeys () {
    ...
    return $("table").att("align", "center")
      .add($("tr")
        .adds([...It.range(1, 6).map(function (n) {
          return $("td").add(Ui.link(mkf(n)).add(
            View.tracker().take("k" + (n)).klass("frame")
          ));
        })]))
      .add($("tr")
        .adds([...It.range(6, 10).map(function (n) {
          return $("td").add(Ui.link(mkf(n)).add(
            View.tracker().take("k" + (n)).klass("frame")
          ));
        })])
  ...
  static loadShow () {
    ...
    let ix = 0;
    View.mkBody($("table").att("align", "center")
      .adds([...It.range(3).map(function () {
        return $("tr").adds([...It.range(3).map(function () {
          const data = Model.mdata().memo()[ix++];
          if (data !== undefined) {
            return $("td")
              .add(Ui.link(() => {
